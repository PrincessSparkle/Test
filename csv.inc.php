...
class CSV
{
    ...
    public function __construct($tempFile, $test_1  = FALSE) //See Source 2
    {
...
// end of csvToArray
        ...
        for ($i = 0; $i < count($types); $i++)
        {
            if ($types[$i] === 'string')
            {
                ...
                $row[$i] = iconv($this->sourceCharSet,
                    $this->dbdatabaseCharSet ."//TRANSLIT", (string) $row[$i]);
    ...
    private function fieldValues($row, $incorrectCommas)
    {
        ...
        for ($i = 1; $i < count($tempRow); $i++)
        {
            ...
            {
                if ($i === 1 || ( substr($tempRow[$i - 1], 0, 1) !== ' ' && $i !== 1 ))
                { //Checking Previous fields
                    $tempRow[$i - 1] = '"'.$tempRow[$i - 1];
                }
                if (substr($tempRow[$i + 1], 0, 1) !== ' ')
                { // checking the next fields for entries in the text block
                    $tempRow[$i] = $tempRow[$i].'"';
                }
            }
        }
        ...
        foreach ($arrIn as $row) {
            ...
            switch ($arrayReturned[0]) {
                ...
                case 'fail':
                    ...
                    break;
                case 'exists':
                    $this->arrayExist[] = $arrayReturned[1];
                    break;
                default:
    ...
    private function print($row) //See source 5
    {
    ...
    public function get_arrayError()
    ...
    public function get_arraySkipped()
    {
    ...
    public function get_arrayExist()
    {
        return (isset($this->arrayExist)) ? $this->arrayExist : NULL;
    }
    ...
    public function get_arraySuccess()
    ...
    public function getOutput()
    {
        ...
        $stringOutput .= str_pad("Stock Proccessed: ", 20).count($this->arrayIn).PHP_EOL;
        ...
        $numErr = count($this->arrayError) + count($this->arrayFail) + count($this->arrayExist);
        ...
        if (count($this->arraySkipped))
        {
            ...
            {
                $stringOutput .= self::print($currVal);
            }
        }
        if (count($this->arrayExist))
        {
            $stringOutput .= str_pad("Skipped Stock (Item already in DB)", 80,
                    "-", STR_PAD_BOTH).PHP_EOL;
            foreach ($this->arrayExist as $currVal)
            {
                $stringOutput .= self::print($currVal);
            }
        }
